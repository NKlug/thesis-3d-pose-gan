...
This means $(\widetilde{X}_r, \widetilde{Y}_ir, \widetilde{Z}_r) = (X_r, Y_r, Z_r)$, that is, the root nodes of both poses are already aligned.
...
\begin{equation}
	f(\widetilde{Z}_i) = \left ( \widetilde{Z}_i \cdot \frac{X_i}{Z_i + dz}\cdot \alpha - X_i \right)^2 + 
					\left ( \widetilde{Z}_i \cdot \frac{Y_i}{Z_i + dz}\cdot \alpha - Y_i \right)^2
					 + ( \widetilde{Z}_i - Z_i )^2 \ .
\end{equation}
Let $a \coloneqq \frac{X_i}{Z_i + dz}\cdot \alpha$ and $b \coloneqq \frac{Y_i}{Z_i + dz}\cdot \alpha$, then one can obtain the same derivative as in Equation \eqref{eq:derivative-minimum-distance}.
Again, this yields
\begin{equation}
